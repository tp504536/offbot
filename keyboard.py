...
qiwi_phone = KeyboardButton('Чек перевод по номеру🥝')
qiw_menu.add(blance_qiwi).add(trans_qiwi).add(up_qiwi).add(qiwi_phone).add(back)
...
card_tink = KeyboardButton('Перевод на карту Сбербанк💂‍♀')
tink_menu.add(blance_tink).add(trans_tink).add(card_tink).add(back)
